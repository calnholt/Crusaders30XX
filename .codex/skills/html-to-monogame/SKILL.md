...
description: Convert an HTML/CSS mockup into a MonoGame implementation plan for Crusaders30XX UI systems. Use when the user provides an HTML mockup path, asks for html-to-monogame, or wants a pixel plan before implementing UI in this project.
---
...
**Coordinate system:** `Game1.VirtualWidth` x `Game1.VirtualHeight` (default **1920x1080**). All pixel tables use screen-space top-left origin unless the UI is card-local (see Appendix A).
...
| Check | Why |
|-------|-----|
| CSS vars use **units** (`1920px` not `1920`) | Unitless values break layout (0x0 viewport) |
| Mockup targets **1920x1080** (or document scale factor) | Match `Game1.VirtualWidth/Height` |
...
- **Do not** use `MouseState` / `GamePad` - input via `UIElement` + `CursorEvents` (project `AGENTS.md`).
...
6. **Pixel position reference** - Full table at 1920x1080 (or note if card-local)
...
- All dimensions x `visualScale` (`vs`).
