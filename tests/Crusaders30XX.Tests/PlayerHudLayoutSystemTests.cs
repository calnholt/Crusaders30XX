...
public class PlayerHudLayoutSystemTests : IDisposable
{
	...
	public void Layout_applies_mockup_rows_overlap_hp_extension_and_pledge_reservation()
	{
		...
		Assert.All(new[] { health, courage, temperance, actionPoint, pledge },
			region => Assert.Equal(44, region.Bounds.Height));
		Assert.Equal(root.Bounds.Y, health.Bounds.Y);
		Assert.Equal(root.Bounds.Y + 44, courage.Bounds.Y);
		Assert.Equal(13, courage.Bounds.Right - temperance.Bounds.X);
		Assert.Equal(13, temperance.Bounds.Right - actionPoint.Bounds.X);
		Assert.Equal(13, actionPoint.Bounds.Right - pledge.Bounds.X);
		Assert.Equal(14, health.Bounds.X - root.Bounds.X);
		Assert.Equal(6, health.Bounds.Right - pledge.Bounds.Right);
		var anchor = GetAnchor(entityManager);
		...
		Assert.Equal(0.13f, anchor.LabelFontScale);
		Assert.Equal(0.20f, anchor.ValueFontScale);
		Assert.Equal(3, anchor.LabelLetterSpacing);
		Assert.Equal(8, anchor.ContentGap);
		Assert.Equal(26, anchor.HealthTrackHeight);
		Assert.Equal(2, anchor.HealthTrackBorderThickness);
		Assert.Equal(16, anchor.TemperanceChunkWidth);
		Assert.Equal(22, anchor.TemperanceChunkHeight);
		Assert.Equal(2, anchor.TemperanceChunkGap);
