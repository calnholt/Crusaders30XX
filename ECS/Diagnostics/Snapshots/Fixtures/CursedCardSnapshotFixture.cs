...
public sealed class CursedCardSnapshotFixture : IDisplaySnapshotFixture
{
    ...
    public void Setup(DisplaySnapshotContext ctx, string[] args)
    {
        ...
        Console.WriteLine(
            $"[DisplaySnapshot] Rendering cursed card: {cardId}, " +
    ...
    private void DrawBackdrop(DisplaySnapshotContext ctx)
    {
        ...
        ctx.SpriteBatch.Draw(_pixel, new Rectangle(0, 0, width, height), new Color(20, 18, 28));
    ...
    private void ParseArguments(string[] args, out string cardId)
    {
        ...
        for (int i = 0; i < (args?.Length ?? 0); i++)
        {
            ...
            {
                throw new DisplaySnapshotSetupException($"Unknown cursed-card option: '{arg}'");
            }
            if (cardIdSet)
            {
                throw new DisplaySnapshotSetupException($"Unexpected cursed-card argument: '{arg}'");
            }
