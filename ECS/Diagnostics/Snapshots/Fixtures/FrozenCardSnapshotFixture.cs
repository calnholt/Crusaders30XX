...
public sealed class FrozenCardSnapshotFixture : IDisplaySnapshotFixture
{
    ...
    private bool _includeBrittle;

    public string Id => "frozen-card";
    ...
    public void Setup(DisplaySnapshotContext ctx, string[] args)
    {
        ...
        Console.WriteLine(
            $"[DisplaySnapshot] Rendering frozen card: {cardId}, " +
            $"scale={_scale:0.##}, rotation={_rotationDegrees:0.##}, brittle={_includeBrittle}");
    }
    ...
    private void DrawBackdrop(DisplaySnapshotContext ctx)
    {
        ...
        ctx.SpriteBatch.Draw(_pixel, new Rectangle(0, 0, width, height), new Color(26, 31, 46));
    ...
    private void ParseArguments(string[] args, out string cardId)
    {
        ...
        for (int i = 0; i < (args?.Length ?? 0); i++)
        {
            ...
            if (string.Equals(arg, "--brittle", StringComparison.OrdinalIgnoreCase))
            {
                _includeBrittle = true;
                continue;
            }
            ...
            {
                throw new DisplaySnapshotSetupException($"Unknown frozen-card option: '{arg}'");
            }
            if (cardIdSet)
            {
                throw new DisplaySnapshotSetupException($"Unexpected frozen-card argument: '{arg}'");
            }
    ...
    private string BuildOutputFileName(string cardId)
    {
        ...
        string rotation = _rotationDegrees.ToString("0.##", CultureInfo.InvariantCulture)
            .Replace('.', '_')
            .Replace('-', 'n');
        return $"{cardId}-s{scale}-r{rotation}" + (_includeBrittle ? "-brittle" : string.Empty);
    }
